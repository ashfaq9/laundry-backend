...
router.post('/', authenticateUser, isAdmin, upload.single('image'),checkSchema(serviceValidation),serviceControl.createService);
router.get('/', serviceControl.getAllServices);
router.get('/:id', serviceControl.getServiceById);
router.put('/:id', authenticateUser, isAdmin, upload.single('image'),checkSchema(serviceValidation),serviceControl.updateService);
